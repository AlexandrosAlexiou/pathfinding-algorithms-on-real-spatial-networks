...
public class Astar {
  ...
  public void computeShortestPaths(int source_id, int target_id) {
    ...
    while (!priorityQueue.isEmpty()) {
      Vertex actualVertex =
          priorityQueue.poll(); // Get the minimum distance vertex from priority queue
      ...
      for (Edge edge : actualVertex.getAdj()) {
        ...
        if (!visited.contains(Integer.parseInt(neighbor.getId()))) {
          ...
          double newDistance =
              actualVertex.getDistance() + edge.getDistance() + neighbor.getPrediction();
    ...
    System.out.println(
        "Shortest Path from " + source_id + " to " + target_id + " = " + shortestPath);
